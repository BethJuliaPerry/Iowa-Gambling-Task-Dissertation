...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/beth/Desktop/dissertation/iowa_gambling_4choice_h-master/IOWA_Gambling_Task_lastrun.py',
...
Instruct1_text = visual.TextStim(win=win, name='Instruct1_text',
    text="In front of you on the screen, there will be 4 decks of cards: 1, 2, 3, and 4. When we begin the game, I want you to select one card at a time by clicking on a card from any deck you choose. \n\nEach time you select a card, the computer will tell you that you won some money. I don't know how much money you will win. You will find out as we go along. \n\nEvery so often, when you click on a card, the computer will tell you that you lost some money. I don't know when you will lose or how much. You will find out as we go along. \n\nPress spacebar to continue. ",
    font='Arial',
...
Instruct2_text = visual.TextStim(win=win, name='Instruct2_text',
    text="You are absolutely free to switch from one deck to the other at any time, and as often as you wish. \n\nThe goal of the game is to win as much money as possible and avoid losing as much money as possible. You won't know when the game will end. Simply keep on playing until the computer stops. I am going to give you 2000 credit, There is a reminder of how much money you have before we see whether you won or lost.\n\nPress spacebar to continue.\n\n",
    font='Arial',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0, 
    color='black', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
text_instr = visual.TextStim(win=win, name='text_instr',
    text='The only hint I can give you, and the most important thing to note is this: Out of these four decks of cards, there are some that are worse than others, and to win you should try to stay away from bad decks. \n\nNo matter how much you find yourself losing, you can still win the game if you avoid the worst decks. \n\nAlso, note that the computer does not change the order of the cards once the game begins. It does not make you lose at random, or make you lose money based on the last card you picked. \n\nIf you have any questions, please ask the researcher and return later. \n\nOtherwise, click the spacebar to begin.',
    font='Arial',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0.0, 
    color=[-1.0000, -1.0000, -1.0000], colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=0.0);
instruct4_resp = keyboard.Keyboard()
...
instruct4_resp.keys = []
instruct4_resp.rt = []
_instruct4_resp_allKeys = []
# keep track of which components have finished
instructions4Components = [text_instr, instruct4_resp]
...
while continueRoutine:
    ...
    # *instruct4_resp* updates
    waitOnFlip = False
    if instruct4_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        instruct4_resp.frameNStart = frameN  # exact frame index
        instruct4_resp.tStart = t  # local t and not account for scr refresh
        instruct4_resp.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(instruct4_resp, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'instruct4_resp.started')
        instruct4_resp.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(instruct4_resp.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(instruct4_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if instruct4_resp.status == STARTED and not waitOnFlip:
        theseKeys = instruct4_resp.getKeys(keyList=['space'], waitRelease=False)
        _instruct4_resp_allKeys.extend(theseKeys)
        if len(_instruct4_resp_allKeys):
            instruct4_resp.keys = _instruct4_resp_allKeys[-1].name  # just the last key pressed
            instruct4_resp.rt = _instruct4_resp_allKeys[-1].rt
            # a response ends the routine
            continueRoutine = False
...
for thisComponent in instructions4Components:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# check responses
if instruct4_resp.keys in ['', [], None]:  # No response was made
    instruct4_resp.keys = None
thisExp.addData('instruct4_resp.keys',instruct4_resp.keys)
if instruct4_resp.keys != None:  # we had a response
    thisExp.addData('instruct4_resp.rt', instruct4_resp.rt)
thisExp.nextEntry()
...
ChoiceBlock = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('GainLosConds.xlsx'),
